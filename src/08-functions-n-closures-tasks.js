...
function getPolynom(...args) {
  return (x) => {
    let y = 0;
    for (let i = 0, k = args.length - 1; i < args.length; i += 1, k -= 1) {
      y += x ** i * args[k];
    }
    return y;
  };
...
function logger(func, logFunc) {
  const argsToStr = (...args) => args.map((arg) => {
    if (Array.isArray(arg)) {
      const items = argsToStr(...arg);
      return `[${items}]`;
    }
  ...
  return (...args) => {
    const funcCallStr = `${func.name}(${argsToStr(...args)})`;
    logFunc(`${funcCallStr} starts`);
    const result = func(...args);
    logFunc(`${funcCallStr} ends`);
    return result;
  };
}
